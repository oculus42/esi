...
/* global it, describe, before, after, localhost */
...
describe('ESI via webserver', function() {
  ...
  it('should process ESI markup', function(done) {
    ...
    var resolve = 'hello';
    var snippet = '<esi:include src="' + localhost + resolve + '"/>' + resolve;
  ...
  it('should let us define esi.vars.custom_name', function(done) {
    ...
    var snippet = '<esi:vars>$(custom)</esi:vars>';
  ...
  it('should replace only in esi tags', function(done) {
    ...
    var snippet = 'foo<esi:vars>$(customOk)</esi:vars>bar<esi:vars>$(customGood)</esi:vars>';
  ...
  describe('should assign default HTTP VARS', function() {
    ...
    [
      ...
      it(vars, function(done) {
        var snippet = '<esi:vars>$(' + vars + ')</esi:vars>';
